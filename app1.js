...
async function getCupos(item) {
  ...
  try {
    // 1. Lanzamos una instancia del navegador
    ...
    $('.contenidotabla tr[onmouseover]').map((index, fila) => {
      ...
      if (nombreGrupo && !isNaN(parseInt(cuposDisponibles))) {
        //console.log(`  -> Grupo: ${nombreGrupo}, Cupos Disponibles: ${cuposDisponibles}`);
    ...
    objeto.horarios.forEach((grupo) => {
      if (parseInt(grupo.cuposDisponibles) >0 && item.gruposBuscados.includes(grupo.nombreGrupo)) {
        ...
        // console.log("------------------------------------------------------------------");
        // console.log(`¡NOTIFICACIÓN ENVIADA! - ${new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota' })}`);
        // console.log(mensaje);
        // console.log("------------------------------------------------------------------");
...
setInterval(() => {
  // console.log("\n=================== Iniciando nuevo ciclo de búsqueda ===================");
